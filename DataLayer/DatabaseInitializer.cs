...
namespace DataLayer
{
    public static class DatabaseInitializer
    {
        public static void Initialize()
        {
            try
            {
                string baseDir = AppDomain.CurrentDomain.BaseDirectory; //output dir
                ...
                //connection string let's goooo
                //integrated security true bylo na wykladzie
                using (var db = new ShopDataBaseDataContext(connectionString))
                {//using bylo na wykladzie
                    if (!db.DatabaseExists())
                    {
                        db.CreateDatabase(); //make db
                    }
                }
